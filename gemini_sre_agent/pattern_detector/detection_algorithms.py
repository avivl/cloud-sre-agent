...
class ConfigurationIssueConfig(DetectionAlgorithmConfig):
    ...
    config_keywords: List[str] = field(default_factory=lambda: [
        "config", "configuration", "settings", "invalid", "missing"
    ])
...
class DependencyFailureConfig(DetectionAlgorithmConfig):
    ...
    dependency_keywords: List[str] = field(default_factory=lambda: [
        "timeout", "connection", "unavailable", "refused", "dns", "network"
    ])
    external_service_indicators: List[str] = field(default_factory=lambda: [
...
class ResourceExhaustionConfig(DetectionAlgorithmConfig):
    ...
    resource_keywords: List[str] = field(default_factory=lambda: [
        "memory", "cpu", "disk", "space", "limit", "quota", "throttle"
    ])
    gradual_onset_indicators: List[str] = field(default_factory=lambda: [
...
class CascadeFailureDetector(BaseDetectionAlgorithm[PatternMatch]):
    ...
    def detect(
        ...
        has_cascade_failure = (
            cascade_triggers or
            (service_impact_triggers and any(
...
class ServiceDegradationDetector(BaseDetectionAlgorithm[PatternMatch]):
    ...
    def detect(
        ...
        for trigger in degradation_triggers:
            ...
            error_logs = len([
                log for log in logs
                if log.service_name in trigger.affected_services and log.severity.upper() in ["ERROR", "CRITICAL"]
            ])
    ...
    def _calculate_confidence(
        ...
        confidence = (
            error_rate_factor * 0.5 +
            log_volume_factor * 0.3 +
            time_factor * 0.2
        )
...
class TrafficSpikeDetector(BaseDetectionAlgorithm[PatternMatch]):
            ...
            if confidence_score >= self.spike_config.min_confidence:
                ...
                patterns.append(PatternMatch(
                    ...
                    evidence={
                        ...
                        "time_window_seconds": (window.end_time - window.start_time).total_seconds(),
                    },
    ...
    def _calculate_confidence(
        ...
        confidence = (
            log_volume_factor * 0.6 +
            time_concentration_factor * 0.4
        )
    ...
    def detect(
        ...
        if confidence_score >= self.config_issue_config.min_confidence:
            ...
            patterns.append(PatternMatch(
                ...
                confidence_score=confidence_score,
                primary_service=primary_service,
                affected_services=list(set(log.service_name or 'unknown' for log in config_logs)),
                severity_level=severity,
                evidence={
...
class DependencyFailureDetector(BaseDetectionAlgorithm[PatternMatch]):
    ...
    def detect(
        ...
        if confidence_score >= self.dependency_config.min_confidence:
            ...
            patterns.append(PatternMatch(
                ...
                confidence_score=confidence_score,
                primary_service=primary_service,
                affected_services=list(set(log.service_name or 'unknown' for log in dependency_logs)),
                severity_level=severity,
                evidence={
    ...
    def _filter_dependency_logs(self, logs: List[LogEntry]) -> List[LogEntry]:
        ...
        for log in logs:
            log_message = (log.error_message or "").lower()
            if any(keyword in log_message for keyword in self.dependency_config.dependency_keywords):
    ...
    def _has_external_services(self, logs: List[LogEntry]) -> bool:
        ...
        for log in logs:
            log_message = (log.error_message or "").lower()
            if any(indicator in log_message for indicator in self.dependency_config.external_service_indicators):
                return True
        return False
...
class ResourceExhaustionDetector(BaseDetectionAlgorithm[PatternMatch]):
    ...
    def detect(
        ...
        if confidence_score >= self.resource_config.min_confidence:
            ...
            patterns.append(PatternMatch(
                ...
                confidence_score=confidence_score,
                primary_service=primary_service,
                affected_services=list(set(log.service_name or 'unknown' for log in resource_logs)),
                severity_level=severity,
                evidence={
    ...
    def _check_gradual_onset(self, logs: List[LogEntry], window: TimeWindow) -> bool:
        ...
        quarters = [
            sorted_logs[i:i+len(sorted_logs)//4] for i in range(0, len(sorted_logs), len(sorted_logs)//4)
        ]
...
class DetectionAlgorithmFactory:
    ...
    @staticmethod
    def create_algorithm(algorithm_type: str, config: Optional[DetectionAlgorithmConfig] = None) -> BaseDetectionAlgorithm[PatternMatch]:
