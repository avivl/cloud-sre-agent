...
from .confidence_calculators import ConfidenceScoreProcessor, ConfidenceRuleFactory
from .models import (
    PatternMatch,
    ThresholdResult,
    TimeWindow,
    ...
    def combine_patterns(
        ...
        for group_patterns in pattern_groups.values():
            if len(group_patterns) == 1:
                ...
                combined_pattern = self._combine_group_patterns(
                    group_patterns, config
                )
                if combined_pattern and combined_pattern.confidence_score >= config.min_ensemble_confidence:
...
class ConsensusVotingStrategy:
    ...
    def combine_patterns(
        ...
        for group_patterns in pattern_groups.values():
            if len(group_patterns) >= 2:
                # Check for consensus
                consensus_pattern = self._find_consensus_pattern(
                    group_patterns, config
                )
    ...
    def _find_consensus_pattern(
        ...
        high_agreement_pairs = [
            (i, j, score) for i, j, score in agreement_scores
        ...
        best_pattern_index = max(
            pattern_agreement_scores.keys(),
            key=lambda k: pattern_agreement_scores[k]
        )
...
class PatternEnsemble:
    ...
    def __init__(
        ...
        self.pattern_matchers = PatternMatcherFactory.create_all_matchers(
            self.config.pattern_configs or {}, 
            confidence_scorer
        )
    ...
    def classify_patterns(
        ...
        for matcher_name, matcher in self.pattern_matchers.items():
            try:
                ...
                self.logger.error(
                    f"[PATTERN_ENSEMBLE] Error in {matcher_name}: {e}"
                )
